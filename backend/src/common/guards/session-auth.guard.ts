...
export class SessionAuthGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        try {
            ...
            if (!session) {
                const cookieKeys = headers.get('cookie') ? headers.get('cookie')?.split(';').map(c => c.split('=')[0].trim()).join(', ') : 'NONE';
                const authHeaderPrefix = headers.get('authorization')?.substring(0, 10) ?? 'NONE';
            ...
            if (error?.response?.message && error.response.message.startsWith('DEBUG:')) {
                throw error;
            }
            throw new UnauthorizedException(`DEBUG: Validation Error. ${error.message}`);
        }
    }
}
